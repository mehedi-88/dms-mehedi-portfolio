...
(function () {
  ...
  typingRow.className = "dms-typing";
  ...
  function showTyping() {
    ...
    // Admin typing indication.
    //TODO: Replace animation with specified one.
    try {
      if (!typingBubbleEl) {
        typingBubbleEl = document.createElement('div');
        typingBubbleEl.className = 'dms-chat__bubble dms-chat__bubble--agent dms-typing-bubble';
        typingBubbleEl.id = 'dms-agent-typing-bubble';
        typingBubbleEl.innerHTML = `
  <div class="dms-typing show">
    <span class="label">Admin is typing</span>
    <span class="dots">
...
`;
      ...
      if (!msgsEl.querySelector('#' + typingBubbleEl.id)) {
        msgsEl.appendChild(typingBubbleEl);
        scrollEnd();
      }
    } catch (err) { console.debug('showTyping bubble error', err); }
  }
  function hideTyping() {
    ...
    } catch (err) { console.debug('hideTyping bubble error', err); }
  }
  ...
  function initSSE() {
    ...
    es.addEventListener("typing", e => {
      try {
        ...
        // debug to help troubleshoot typing delivery
        console.debug("SSE typing event:", d);
        if (d.who === "agent" || d.who === "bot") {
          d.state ? showTyping() : hideTyping();
        }
      } catch (err) { console.debug('typing parse error', err); }
    });
